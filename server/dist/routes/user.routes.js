...
// 确保头像目录存在（从项目根目录开始计算）
const avatarDir = path_1.default.join(__dirname, '../../public/avatar');
...
// 上传头像
router.post('/avatar', auth_middleware_1.authenticate, upload.single('avatar'), async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({
                code: 401,
        ...
        // 更新数据库中的头像URL
        await (0, auth_service_1.updateUserAvatar)(req.user.id, avatarUrl);
        res.json({
            code: 200,
            msg: '上传成功',
            data: {
...
router.put('/profile', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const updatedUser = await (0, auth_service_1.updateUserProfile)(req.user.id, updateData);
        res.json({
            code: 200,
            msg: '更新成功',
            data: {
...
// 关注用户
router.post('/follow/:userId', auth_middleware_1.authenticate, async (req, res) => {
    try {
        if (!req.user) {
...
// 取消关注
router.delete('/follow/:userId', auth_middleware_1.authenticate, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ code: 401, msg: '未授权' });
        }
...
// 检查是否关注某用户
router.get('/follow/status/:userId', auth_middleware_1.authenticate, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ code: 401, msg: '未授权' });
        }
...
// 获取用户的粉丝数和关注数
router.get('/stats/:userId', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const targetUserId = req.params.userId;
        const prisma = (await Promise.resolve().then(() => __importStar(require('../prisma/client')))).default;
